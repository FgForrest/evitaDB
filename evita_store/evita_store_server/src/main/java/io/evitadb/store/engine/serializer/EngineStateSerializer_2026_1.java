...
/**
 * Backward-compatible {@link Serializer} implementation that reads {@link EngineState} persisted before the
 * `missingCatalogs` array was introduced. The old format has three catalog arrays (active, inactive, read-only) with
 * cumulative-checksum WAL references; this serializer substitutes an empty `missingCatalogs` array so callers see
 * the new record shape transparently.
 *
...
public class EngineStateSerializer_2026_1 extends Serializer<EngineState> {
	...
	public EngineState read(Kryo kryo, Input input, Class<? extends EngineState> aClass) {
		...
		// Pre-Part-C format has no missing-catalogs array — substitute an empty one so the record shape
		// remains uniform for callers.
