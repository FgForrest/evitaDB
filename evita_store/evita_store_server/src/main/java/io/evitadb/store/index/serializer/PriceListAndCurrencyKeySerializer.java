...
public class PriceListAndCurrencyKeySerializer extends Serializer<PriceIndexKey> {

	@Override
	public void write(Kryo kryo, Output output, PriceIndexKey key) {
		output.writeString(key.getPriceList());
		kryo.writeObject(output, key.getCurrency());
		output.writeVarInt(key.getRecordHandling().ordinal(), true);
	}
