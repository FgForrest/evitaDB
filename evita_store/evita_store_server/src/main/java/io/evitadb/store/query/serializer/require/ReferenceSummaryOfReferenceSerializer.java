...
/**
 * This {@link Serializer} implementation reads/writes {@link ReferenceSummaryOfReference} from/to binary format.
 * The wire format follows the constraint's logical shape:
 *
 * 1. reference name (non-null string)
 * 2. statistics depth ({@link FacetStatisticsDepth}, non-null)
 * 3. additional children: filterBy, filterGroupBy, orderBy, orderGroupBy (each nullable)
 * 4. fetch children: entityFetch, entityGroupFetch (each nullable)
 * 5. histogram statistics: count + N {@link ReferenceHistogramStatistics} children
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2026
 */
@RequiredArgsConstructor
public class ReferenceSummaryOfReferenceSerializer extends Serializer<ReferenceSummaryOfReference> {

	@Override
	public void write(Kryo kryo, Output output, ReferenceSummaryOfReference object) {
	...
	@Override
	public ReferenceSummaryOfReference read(Kryo kryo, Input input, Class<? extends ReferenceSummaryOfReference> type) {
