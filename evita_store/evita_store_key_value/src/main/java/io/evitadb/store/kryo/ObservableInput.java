...
public class ObservableInput<T extends InputStream> extends Input {
	 ...
	 *
	 * A concurrent write-and-tail stress run looks like the counterpart to that test and is not one: measured
	 * against the same counterfactual on an idle box it stayed green over three consecutive runs, so the skew it
	 * is meant to provoke went unprovoked. Do not read a green stress run as cover for a change here.
	 *
