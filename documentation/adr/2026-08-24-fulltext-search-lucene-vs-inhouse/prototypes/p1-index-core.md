...
- **Real data** gets into the measurement by two routes: the JMH states of the `Senesi*` family
  (`io/evitadb/performance/senesi/`), and `WalReplayBenchmark`, which boots an embedded instance from a
...
**The e-commerce profile.** The real senesi / decodoma catalog. It is **not** in the repo — it is supplied
