...
<dl>
    ...
    <dd>
        povinný výčtový argument představující rozsah, na který se aplikují podmínky řazení ve druhém a dalších argumentech
    </dd>
    <dt>orderConstraint:any+</dt>
    <dd>
        jedna nebo více povinných podmínek řazení, propojených logickým operátorem, které se používají k řazení entit pouze v konkrétním rozsahu
    </dd>
</dl>

Kontejner řazení `inScope` (<LS to="e,j,r,g"><SourceClass>evita_query/src/main/java/io/evitadb/api/query/ordering/OrderInScope.java</SourceClass></LS>
<LS to="c"><SourceClass>EvitaDB.Client/Queries/Ordering/OrderInScope.cs</SourceClass></LS>) slouží
k omezení podmínek řazení tak, aby se vztahovaly pouze na konkrétní rozsah.

Dotazovací engine evitaDB je striktní ohledně indexů a neumožňuje řadit nebo třídit podle dat (atributů, referencí
atd.), pro která nebyl předem připraven index (snaží se tak zabránit situacím, kdy by úplné prohledání zhoršilo výkon dotazu). Rozsahy nám naopak umožňují zbavit se zbytečných indexů, pokud víme, že je nebudeme potřebovat
(archivovaná data se neočekává, že budou dotazována tak často jako živá data), a uvolnit tak prostředky pro důležitější úlohy.

[Podmínka řazení inScope](#v-rozsahu) nám umožňuje dotazovat entity v obou rozsazích najednou,
což by nebylo možné, pokud bychom nemohli určit, která podmínka řazení se vztahuje na který rozsah. Kontejner `inScope`
...
Například v naší demo databázi máme v archivu indexováno pouze několik atributů – konkrétně `url` a `code` a několik dalších. V archivu však žádný atribut nemá index pro řazení. Pokud bychom tedy vybírali několik entit z obou rozsahů a pokusili se je seřadit podle `name`, dostali bychom chybu. Pokud ale použijeme kontejner `inScope`, můžeme entity v živém rozsahu řadit podle `name` a archivované entity budou seřazeny podle pořadí v zadání dotazu.
