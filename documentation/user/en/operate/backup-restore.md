...
A failure *inside* the swap itself is the one case that leaves the target catalog in neither state. Past a certain point the swap cannot be taken back, and a failure beyond it leaves the catalog refusing sessions and reporting itself as corrupted until the server is restarted - after which it comes back carrying the restored data, because the swap had in fact got far enough. A restore that reports a failure is therefore worth checking on before it is repeated.
