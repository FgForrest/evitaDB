...
A failure *inside* the swap itself is the one case the swap cannot take back. Past a certain point the restored catalog's storage has already been relabelled, while the engine has not yet recorded the exchange, and there is no way back to either end state. The catalog you were replacing keeps the contents it already had, and the restored copy is discarded rather than published - it is reported as corrupted under the temporary name and then removed. Restarting the server does not bring it back, because the restored data is recovered by name and that name never changed hands. The snapshot is kept in this case, so the restore can be repeated from it.
