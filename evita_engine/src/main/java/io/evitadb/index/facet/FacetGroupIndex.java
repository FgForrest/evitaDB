...
public class FacetGroupIndex implements TransactionalLayerProducer<FacetGroupIndexChanges, Map<Integer, Bitmap>>, IndexDataStructure {
	...
	public boolean addFacet(int facetPrimaryKey, int entityPrimaryKey) {
		final FacetGroupIndexChanges txLayer = TransactionalMemory.getTransactionalMemoryLayer(this);
	...
	public boolean removeFacet(int facetPrimaryKey, int entityPrimaryKey) {
		...
		Assert.notNull(facetIdIndex, "Facet `" + facetPrimaryKey + "` not found in index (group: `" + groupId + "`)!");
	...
	public Map<Integer, Bitmap> getAsMap() {
		...
		for (Entry<Integer, FacetIdIndex> entry : facetIdIndexes.entrySet()) {
	...
	public String toString() {
		final StringBuilder sb = new StringBuilder(
			ofNullable(groupId).map(it -> "GROUP " + it).orElse("[NO_GROUP]") + ":\n"
		);
		this.facetIdIndexes
			.keySet()
			.stream()
