...
public final class Transaction implements TransactionContract {
	 ...
	 */
	public static StoragePartPersistenceService<StorageDescriptor> createTransactionalPersistenceService(
		@Nonnull StoragePartPersistenceService<StorageDescriptor> storagePartPersistenceService
	) {
		final Transaction transaction = CURRENT_TRANSACTION.get();
		if (transaction != null && !transaction.isReplay()) {
			final StoragePartPersistenceService<StorageDescriptor> transactionalService = storagePartPersistenceService.createTransactionalService(
