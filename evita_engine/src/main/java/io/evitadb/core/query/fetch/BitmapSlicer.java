...
class BitmapSlicer {
	 ...
	 * winners — no over-fetch, no under-fetch (issue #1177).
	 *
	...
	/**
	 * Sorts the candidates in-place by the comparator chain. Mirrors the algorithm used by
	 * {@code EntityDecorator#sortAndFilterSubList} so the pre-fetch slice agrees with the post-fetch sort:
	 * sort the prefix with the primary comparator, then advance past the sorted range and recurse with the
	 * next-in-chain comparator on whatever it couldn't rank (returned via {@code getNonSortedReferenceCount}).
	 *
	 * The non-sorted count returned by some comparators (notably `EntityNestedQueryComparator`) is a
	 * monotonically growing cross-entity accumulator — the comparator never resets its internal
	 * `nonSortedReferences` set between source-entity passes. We therefore snapshot the count
	 * *before* `Arrays.sort` and use the per-pass delta to derive the unsorted-tail length. The
	 * delta is additionally clamped to `[0, end - start]` so a malformed comparator (delta larger
	 * than the slice it just sorted) cannot produce a negative `start` and trip `Arrays.sort` with
	 * a backwards range. The shared accumulator behavior is intentional — do not "fix" it by
	 * resetting the comparator between entities; the comparator's lifecycle is owned by the caller.
	 *
	...
	public Bitmap slice(@Nonnull Bitmap primaryKeys, @Nonnull Strip strip) {
		...
		return new ArrayBitmap(
			primaryKeys.getRange(
				offsetAndLimit.offset(),
				Math.min(strip.getOffset() + offsetAndLimit.limit(), primaryKeys.size())
			)
		);
	}
