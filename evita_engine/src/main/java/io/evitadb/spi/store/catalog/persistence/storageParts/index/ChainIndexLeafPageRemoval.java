...
/**
 * A flush-time instruction to REMOVE a single granular {@link io.evitadb.index.attribute.ChainIndex} value-tree leaf
 * page that a leaf drop (or a `PAGED -> SINGLE` collapse) freed this commit. Removing it is necessary, not optional: the
 * append-only OffsetIndex never reclaims a record that is neither superseded (page sequences are advance-only and never
 * re-keyed) nor explicitly removed, so an unreferenced leaf page would otherwise be copied forward by every compaction
 * forever.
 *
 * Like {@link ChainIndexLeafPagePart} on the write path, this carries the sub-index `(entityIndexPrimaryKey,
 * attributeKey)` identity rather than a pre-resolved `streamId`: the writable {@link KeyCompressor} lives store-side, so
 * the target primary key `pack(streamId, pageSequence)` is resolved store-side in {@link #computeUniquePartIdAndSet}
 * (the `streamId` is already registered — the page existed before it was freed). The part carries no payload and is
 * never written, so it needs no Kryo serializer; the flush drain resolves its key and removes the leaf page.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2026
 */
public class ChainIndexLeafPageRemoval implements DeferredRemovalStoragePart {
	@Serial private static final long serialVersionUID = -7194623058374619205L;
	...
	/**
	 * The attribute + index-type identity of the sub-index (CHAIN-typed) — used to resolve the freed page's `streamId`
	 * store-side.
	 */
	 ...
	 */
	public ChainIndexLeafPageRemoval(
	...
	public Class<? extends StoragePart> removedContainerType() {
		return ChainIndexLeafPagePart.class;
	}
}
