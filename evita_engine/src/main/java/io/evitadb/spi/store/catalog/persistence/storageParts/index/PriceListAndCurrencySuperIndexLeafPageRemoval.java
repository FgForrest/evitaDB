...
/**
 * A flush-time instruction to REMOVE a single granular super-price-index leaf page that a leaf merge (or a
 ...
 *
 * Like {@link PriceListAndCurrencySuperIndexLeafPagePart} on the write path, this carries the sub-index
 * `(entityIndexPrimaryKey, priceIndexKey)` identity rather than a pre-resolved `streamId`: the writable
 * {@link KeyCompressor} lives store-side, so the target primary key `join(streamId, pageSequence)` is resolved store-side
 * in {@link #computeUniquePartIdAndSet} (the `streamId` is already registered — the page existed before it was freed).
 * The part carries no payload and is never written, so it needs no Kryo serializer; the flush drain resolves its key and
 * removes the leaf page.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2026
 */
public class PriceListAndCurrencySuperIndexLeafPageRemoval implements DeferredRemovalStoragePart {
	@Serial private static final long serialVersionUID = 5193847261058372940L;
	...
	/**
	 * The price list and currency identity of the sub-index — used to resolve the freed page's `streamId` store-side.
	 */
	...
	/**
	 * The resolved storage-part primary key `join(streamId, pageSequence)`; `null` until
	 ...
	 */
	public PriceListAndCurrencySuperIndexLeafPageRemoval(
	...
	public long computeUniquePartIdAndSet(@Nonnull KeyCompressor keyCompressor) {
		...
		final int streamId = keyCompressor.getId(new PriceLeafStreamKey(this.entityIndexPrimaryKey, this.priceIndexKey));
		final long computedUniquePartId = PriceListAndCurrencySuperIndexLeafPagePart.computeUniquePartId(
			streamId, this.pageSequence
		);
	...
	public Class<? extends StoragePart> removedContainerType() {
		return PriceListAndCurrencySuperIndexLeafPagePart.class;
	}
}
