...
/**
 * A flush-time instruction to REMOVE a single granular OWNER-mode {@link io.evitadb.index.attribute.SortIndex} value-tree
 ...
 * advance-only and never re-keyed) nor explicitly removed, so an unreferenced leaf page would otherwise be copied forward
 * by every compaction forever.
 *
 * Like {@link SortIndexLeafPagePart} on the write path, this carries the sub-index `(entityIndexPrimaryKey, attributeKey)`
 ...
 */
public class SortIndexLeafPageRemoval implements DeferredRemovalStoragePart {
	@Serial private static final long serialVersionUID = -5837461029384756102L;
	 ...
	 */
	public SortIndexLeafPageRemoval(
	...
	public Class<? extends StoragePart> removedContainerType() {
		return SortIndexLeafPagePart.class;
	}
}
