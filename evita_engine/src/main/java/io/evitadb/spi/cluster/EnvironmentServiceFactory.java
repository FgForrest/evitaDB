 ...
 * an environment service - namely mock and k8s. Therefore, we used {@link ServiceLoader} pattern to dynamically
 * locate proper implementation of this interface and link these modules in runtime.
 *
 * The environment service itself also needs initial configuration from the main evitaDB class, and therefore we need
...
public interface EnvironmentServiceFactory {
	...
	/**
	 * Returns the configuration class for this cluster service type.
	...
	/**
	 * Creates new instance of {@link EnvironmentService}.
	 *
	 * @param clusterOptions cluster configuration options
	 * @return configured environment service instance
	 */
	@Nonnull
	EnvironmentService create(@Nonnull ClusterOptions clusterOptions);
