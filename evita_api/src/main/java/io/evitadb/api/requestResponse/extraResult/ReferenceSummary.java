...
/**
 * This DTO allows returning summary of all references that match query filter excluding those inside
 * {@link UserFilter}. DTO contains information about reference groups and individual references in them
 * as well as appropriate statistics for them.
 *
 * Instance of this class is returned in {@link EvitaResponse#getExtraResult(Class)} when
 * {@link io.evitadb.api.query.require.ReferenceSummary} require query is used in the query.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2021
 */
@ThreadSafe
public class ReferenceSummary implements EvitaResponseExtraResult, PrettyPrintable {
	@Serial private static final long serialVersionUID = -5622027322997919409L;
	/**
	 * Contains statistics of references aggregated into reference groups ({@link Reference#getGroup()}).
	 */
	...
	public ReferenceSummary(@Nonnull Map<String, Collection<ReferenceGroupStatistics>> referenceStatistics) {
		final Map<String, ReferenceStatistics> result = createLinkedHashMap(referenceStatistics.size());
		for (Entry<String, Collection<ReferenceGroupStatistics>> entry : referenceStatistics.entrySet()) {
			final ReferenceGroupStatistics nonGroupedStatistics = entry.getValue()
				.stream()
							...
							Collectors.toMap(
								...
								(o, o2) -> {
									throw new GenericEvitaInternalError(
										"Unexpected duplicate reference group statistics."
									);
								},
								LinkedHashMap::new
							)
	...
	public ReferenceSummary(@Nonnull Collection<ReferenceGroupStatistics> referenceStatistics) {
		this.referenceStatistics = Collections.unmodifiableMap(
			referenceStatistics
				.stream()
				.collect(
					Collectors.groupingBy(
						ReferenceGroupStatistics::getReferenceName
					)
				)
				.entrySet()
				.stream()
							...
							it.getValue().stream().filter(group -> group.getGroupEntity() != null)
								.collect(
									Collectors.toMap(
										...
										(o, o2) -> {
											throw new GenericEvitaInternalError(
												"There is already group for reference `" + it.getKey() +
	...
	/**
	 * Returns statistics for reference group with passed referenced type.
	 */
	@Nullable
	public ReferenceGroupStatistics getReferenceGroupStatistics(@Nonnull String referencedEntityType) {
	...
	/**
	 * Returns statistics for reference group with passed referenced type and primary key of the group.
	 */
	@Nullable
	public ReferenceGroupStatistics getReferenceGroupStatistics(@Nonnull String referencedEntityType, int groupId) {
		return ofNullable(this.referenceStatistics.get(referencedEntityType))
			.map(it -> it.getReferenceGroupStatistics(groupId))
			.orElse(null);
	}
	...
	public boolean equals(Object o) {
		...
		final ReferenceSummary that = (ReferenceSummary) o;
	...
	public String toString() {
		return "Reference summary with: " + this.referenceStatistics.size() + " references";
	}
	 ...
	 * @param context Pretty printing context.
	...
	/**
	 * Pretty prints the reference summary using the provided renderers for groups and facets.
	 *
	 * @param groupRenderer renderer function for group statistics
	 * @param facetRenderer renderer function for facet statistics
	 * @return formatted string representation of the reference summary
	 */
	public String prettyPrint(
	...
	private record ReferenceStatistics(
		@Nullable ReferenceGroupStatistics nonGroupedStatistics,
		@Nonnull Map<Integer, ReferenceGroupStatistics> groupedStatistics
	) {

		@Nullable
		public ReferenceGroupStatistics getReferenceGroupStatistics(int groupId) {
		...
		public boolean equals(Object o) {
			...
			final Map<Integer, ReferenceGroupStatistics> statistics = groupedStatistics();
			final Map<Integer, ReferenceGroupStatistics> thatStatistics = that.groupedStatistics();
			if (statistics.size() != thatStatistics.size()) {
				return false;
			} else {
				final Iterator<Entry<Integer, ReferenceGroupStatistics>> it = statistics.entrySet().iterator();
				final Iterator<Entry<Integer, ReferenceGroupStatistics>> thatIt = thatStatistics.entrySet().iterator();
				while (it.hasNext()) {
					final Entry<Integer, ReferenceGroupStatistics> entry = it.next();
					final Entry<Integer, ReferenceGroupStatistics> thatEntry = thatIt.next();
	...
	/**
	 * This DTO contains information about single reference group and statistics of the facets that relate to it.
	 */
	@Data
	public static class ReferenceGroupStatistics implements Serializable {
		...
		public boolean equals(Object o) {
			if (this == o) return true;
			if (o == null || getClass() != o.getClass()) return false;
			final ReferenceGroupStatistics that = (ReferenceGroupStatistics) o;
	 ...
	 */
	private static final class PrettyPrintingContext {
		/**
