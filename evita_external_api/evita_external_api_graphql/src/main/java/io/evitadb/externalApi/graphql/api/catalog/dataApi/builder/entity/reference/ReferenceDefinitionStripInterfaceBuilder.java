...
/**
 * Builds interfaces from {@link ReferenceDefinitionStripDescriptor} for a specific reference definition.
 *
 * @author Lukáš Hornych, FG Forrest a.s. (c) 2026
 */
@RequiredArgsConstructor
public class ReferenceDefinitionStripInterfaceBuilder {
	...
	@Nonnull private final ReferenceWithReferencedEntityStripInterfaceBuilder referenceWithReferencedEntityStripInterfaceBuilder;
	...
	public GraphQLInterfaceType getOrBuild(
		...
		return this.buildingContext.getOrComputeReferenceDefinitionStripInterface(
			key,
			() -> {
				final String interfaceName = ReferenceDefinitionStripDescriptor.THIS_INTERFACE.name(
					referenceSchema.getReferencedEntityType(),
					Long.toHexString(key.toHash())
				);

				final GraphQLInterfaceType.Builder interfaceBuilder = ReferenceDefinitionStripDescriptor.THIS_INTERFACE
				...
				final GraphQLInterfaceType[] interfaces = new GraphQLInterfaceType[] {
					this.referenceStripInterfaceBuilder.getOrBuild(),
					this.referenceWithReferencedEntityStripInterfaceBuilder.getOrBuild(referenceSchema)
				};
	...
	private String constructDescription(@Nonnull ReferenceSchemaContract referenceSchema) {
		...
		if (referencedGroupType != null && hasAttributes) {
			return ReferenceDefinitionStripDescriptor.THIS_INTERFACE.description(
	...
	private GraphQLFieldDefinition buildDataField(
		...
		return ReferenceDefinitionStripDescriptor.DATA
