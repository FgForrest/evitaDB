...
/**
 * Subscription data fetcher for listening to {@link ChangeCatalogCapture} for catalog data changes inside particular catalog.
 *
 * @author Lukáš Hornych, FG Forrest a.s. (c) 2025
 */
public class OnCatalogDataChangeCaptureSubscribingDataFetcher
	extends ChangeCaptureSubscribingDataFetcher<ChangeCatalogCapture> {

	public OnCatalogDataChangeCaptureSubscribingDataFetcher(@Nonnull Evita evita) {
		super(evita);
	}
	...
	protected Publisher<ChangeCatalogCapture> createPublisher(@Nonnull DataFetchingEnvironment environment) {
		...
		final List<String> containerName = environment.getArgument(OnCatalogDataChangeHeaderDescriptor.CONTAINER_NAME.name());
