...
public interface DeleteEntitiesMutationHeaderDescriptor {
	...
	PropertyDescriptor FILTER_BY = PropertyDescriptor.builder()
		.name("filterBy")
		.description("""
			Complex filter query to filter entities by.
			""")
		// type is expected to be a filterBy object
		.build();
	PropertyDescriptor ORDER_BY = PropertyDescriptor.builder()
		.name("orderBy")
		.description("""
			Complex order query to order entities by.
			""")
		// type is expected to be a orderBy object
		.build();
	PropertyDescriptor OFFSET = PropertyDescriptor.builder()
		.name("offset")
		.description("""
			Argument for adjusting default offset of entities to delete.
			""")
	...
	PropertyDescriptor LIMIT = PropertyDescriptor.builder()
		.name("limit")
		.description("""
			Argument for adjusting default number of maximum entities to delete.
			""")
		.type(nullable(Integer.class))
		.build();
	PropertyDescriptor PRIMARY_KEY = PropertyDescriptor.builder()
		.name("primaryKey")
		.description("""
            Identification of single entity to delete.
			""")
	...
	PropertyDescriptor REQUIRE = PropertyDescriptor.builder()
		.name("require")
		.description("""
			Limited require query to specify content of deleted entity
			""")
