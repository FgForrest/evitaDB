		...
		if (isJavaTypeGeneric(valueParameterType)) {
			...
			if (Locale.class.equals(valueParameterType) || Locale.class.equals(valueParameterType.getComponentType())) {
				...
				return DataTypesConverter.wrapGraphQLComponentType(
					typeRef(ENTITY_LOCALE_ENUM.name(sharedContext.getEntitySchemaOrElseThrow(buildContext.dataLocator().entityType()))),
					valueParameterType,
				...
				return DataTypesConverter.wrapGraphQLComponentType(
					typeRef(ENTITY_CURRENCY_ENUM.name(sharedContext.getEntitySchemaOrElseThrow(buildContext.dataLocator().entityType()))),
					valueParameterType,
					canBeRequired && valueParameter.required()
				);
			} else {
