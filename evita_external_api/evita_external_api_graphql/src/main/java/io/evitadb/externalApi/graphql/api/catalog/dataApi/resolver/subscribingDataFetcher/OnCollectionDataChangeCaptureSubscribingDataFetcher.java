...
import io.evitadb.externalApi.graphql.api.catalog.dataApi.model.OnCollectionDataChangeHeaderDescriptor;
...
/**
 * Subscription data fetcher for listening to {@link ChangeCatalogCapture} for collection data changes inside particular catalog.
 *
 * @author Lukáš Hornych, FG Forrest a.s. (c) 2023
 */
public class OnCollectionDataChangeCaptureSubscribingDataFetcher
	...
	public OnCollectionDataChangeCaptureSubscribingDataFetcher(@Nonnull Evita evita, @Nonnull EntitySchemaContract entitySchema) {
		super(evita);
	...
	protected Publisher<ChangeCatalogCapture> createPublisher(@Nonnull DataFetchingEnvironment environment) {
		...
		final List<ContainerType> containerType = environment.getArgument(OnCollectionDataChangeHeaderDescriptor.CONTAINER_TYPE.name());
		final List<String> containerName = environment.getArgument(OnCollectionDataChangeHeaderDescriptor.CONTAINER_NAME.name());
		final Integer entityPrimaryKey = environment.getArgument(OnCollectionDataChangeHeaderDescriptor.ENTITY_PRIMARY_KEY.name());
