...
import io.evitadb.externalApi.exception.ParameterLimitException;
import io.evitadb.externalApi.exception.UrlDecodeException;
...
public class URLUtils {
	...
	/*private static final QueryStringParser QUERY_STRING_PARSER = new QueryStringParser('&', false) {
		@Override
		void handle(HttpService httpService, String key, String value) {
			httpService.addQueryParam(key, value);
		}
	};*/
	/*private static final QueryStringParser PATH_PARAM_PARSER = new QueryStringParser(';', true) {
		@Override
		void handle(HttpService httpService, String key, String value) {
			httpService.addPathParam(key, value);
		}
	};*/
	...
	/*public static void parseQueryString(final String string, final HttpService httpService, final String charset, final boolean doDecode, int maxParameters) throws ParameterLimitException {
		QUERY_STRING_PARSER.parse(string, httpService, charset, doDecode, maxParameters);
	}*/

	/*@Deprecated
	public static void parsePathParms(final String string, final HttpService httpService, final String charset, final boolean doDecode, int maxParameters) throws ParameterLimitException {
		parsePathParams(string, httpService, charset, doDecode, maxParameters);
	}

	public static int parsePathParams(final String string, final HttpService httpService, final String charset, final boolean doDecode, int maxParameters) throws ParameterLimitException {
		return PATH_PARAM_PARSER.parse(string, httpService, charset, doDecode, maxParameters);
	}*/
				...
				try {
					...
					while ((i < numChars)) {
						if (c == '%') {
							...
							if ((i + 2) >= s.length()) {
								throw UrlDecodeException.failedToDecodeURL(s, enc);
							}
					...
					throw UrlDecodeException.failedToDecodeURL(s, enc, e);
	...
	private abstract static class QueryStringParser {
		...
		int parse(final String string, final HttpService httpService, final String charset, final boolean doDecode, int max) throws ParameterLimitException {
			int count = 0;
			int i = 0;
			try {
				int stringStart = 0;
				...
				for (i = 0; i < string.length(); ++i) {
					...
					if (c == '=' && attrName == null) {
						...
						if (attrName != null) {
							handle(httpService, decode(charset, attrName, doDecode), decode(charset, string.substring(stringStart, i), doDecode));
							if(++count > max) {
								throw ParameterLimitException.tooManyParameters(max);
							}
						} else if (stringStart != i) { // Ignore if attrName == null and stringStart == i because it means both key and value are empty.
							handle(httpService, decode(charset, string.substring(stringStart, i), doDecode), "");
							if(++count > max) {
								throw ParameterLimitException.tooManyParameters(max);
							}
						}
		...
		abstract void handle(final HttpService httpService, final String key, final String value);
	}
	...
	public static boolean getSlashDecodingFlag() {
		// TODO tpz: flags
		return getSlashDecodingFlag(true, true);
	}
