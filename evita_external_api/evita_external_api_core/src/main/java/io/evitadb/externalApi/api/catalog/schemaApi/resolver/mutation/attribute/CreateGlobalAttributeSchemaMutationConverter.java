...
public class CreateGlobalAttributeSchemaMutationConverter
	...
	protected CreateGlobalAttributeSchemaMutation convertFromInput(@Nonnull Input input) {
		...
		final ScopedAttributeFilterAccelerators[] acceleratorsInScopes = input.getOptionalProperty(
			CreateGlobalAttributeSchemaMutationDescriptor.ACCELERATORS_IN_SCOPES.name(),
			new PropertyObjectListMapper<>(
				getMutationName(),
				getExceptionFactory(),
				CreateGlobalAttributeSchemaMutationDescriptor.ACCELERATORS_IN_SCOPES,
		...
		return new CreateGlobalAttributeSchemaMutation(
			...
			input.getProperty(CreateGlobalAttributeSchemaMutationDescriptor.CONFLICT_RESOLUTION_OVERRIDE, ConflictResolutionOverride.INHERITED)
		);
	}
}
