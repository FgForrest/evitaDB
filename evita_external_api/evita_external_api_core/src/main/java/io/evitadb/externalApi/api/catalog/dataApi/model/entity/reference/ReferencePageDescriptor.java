...
/**
 * Describes a paginated list of {@link ReferenceDescriptor#THIS_INTERFACE}.
 *
 * @author Lukáš Hornych, FG Forrest a.s. (c) 2026
 */
public interface ReferencePageDescriptor extends PaginatedListDescriptor {

	PropertyDescriptor DATA = PropertyDescriptor.from(PaginatedListDescriptor.DATA)
		.type(nonNullListRef(ReferenceDescriptor.THIS_INTERFACE))
		.build();

	ObjectDescriptor THIS_INTERFACE = ObjectDescriptor.implementing(PaginatedListDescriptor.THIS_INTERFACE)
		.name("ReferencePage")
		.description("Page of references according to pagination rules in input query.")
		.staticProperty(DATA)
		.build();
}
