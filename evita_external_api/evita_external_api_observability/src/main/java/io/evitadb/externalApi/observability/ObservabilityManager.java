...
import io.evitadb.externalApi.observability.metric.CustomEventProviderRegistry;
 ...
 * and to be registered in the {@link CustomEventProviderRegistry}.
 *
...
public class ObservabilityManager {
	...
	private static void registerJfrEvents(@Nonnull String[] allowedEvents) {
		for (String event : Arrays.stream(allowedEvents).filter(x -> !x.startsWith("jdk.")).toList()) {
			if (event.endsWith(".*")) {
				final Set<Class<? extends CustomMetricsExecutionEvent>> classes = CustomEventProviderRegistry.getEventClassesFromPackage(event);
				...
				final Class<? extends CustomMetricsExecutionEvent> clazz = CustomEventProviderRegistry.getEventClass(event);
				if (!Modifier.isAbstract(clazz.getModifiers())) {
					FlightRecorder.register(clazz);
				}
			}
