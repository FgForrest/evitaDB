...
@Tag(FACET)
@Tag(TRANSACTION)
class LongRunningSavepointFacetGroupIndexTest implements TimeBoundedTestSupport {
	...
	void shouldRollBackFacetGroupIndex(@Nonnull GenerationalTestInput input) {
		runFor(input, 1000, 0L, (random, iteration) -> {
			final FacetGroupState state = new FacetGroupState(random);
			assertSavepointRollbackRestores(
				state.index,
				tested -> state.applyRandomMutations(random, 1 + random.nextInt(MAX_OPS)),
				LongRunningFacetGroupIndexTest::snapshot,
				tested -> {
	...
	void shouldCommitFacetGroupIndex(@Nonnull GenerationalTestInput input) {
		runFor(input, 1000, 0L, (random, iteration) -> {
			final FacetGroupState state = new FacetGroupState(random);
	 ...
	 * transaction.
	 */
	private static final class FacetGroupState {
