...
@Tag(REFERENCE)
@Tag(TRANSACTION)
class LongRunningSavepointFacetReferenceIndexTest implements TimeBoundedTestSupport {
	...
	void shouldRollBackFacetReferenceIndex(@Nonnull GenerationalTestInput input) {
		runFor(input, 1000, 0L, (random, iteration) -> {
			final FacetState state = new FacetState(random);
			assertSavepointRollbackRestores(
				state.index,
				tested -> state.applyRandomMutations(random, 1 + random.nextInt(MAX_OPS)),
				LongRunningFacetReferenceIndexTest::snapshot,
				tested -> {
	...
	void shouldCommitFacetReferenceIndex(@Nonnull GenerationalTestInput input) {
		runFor(input, 1000, 0L, (random, iteration) -> {
			final FacetState state = new FacetState(random);
	 ...
	 */
	private static final class FacetState {
