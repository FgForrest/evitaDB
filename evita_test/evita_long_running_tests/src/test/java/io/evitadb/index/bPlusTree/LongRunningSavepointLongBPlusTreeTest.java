...
@Tag(DATA_TYPE)
@Tag(TRANSACTION)
class LongRunningSavepointLongBPlusTreeTest implements TimeBoundedTestSupport {
	...
	void shouldRollBackLongTreeToSavepoint(@Nonnull GenerationalTestInput input) {
		runFor(
			input,
			1000,
			0L,
			(random, iteration) -> {
				final TransactionalLongBPlusTree<Integer> tree = newSeededTree(random);
				assertSavepointRollbackRestores(
					tree,
					tested -> applyRandomOps(tested, random, 1 + random.nextInt(10)),
					LongRunningSavepointLongBPlusTreeTest::readContents,
					tested -> {
	...
	void shouldCommitLongTreeSavepoint(@Nonnull GenerationalTestInput input) {
		runFor(
			input,
			1000,
			0L,
