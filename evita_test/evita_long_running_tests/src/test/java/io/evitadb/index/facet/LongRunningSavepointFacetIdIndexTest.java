...
/**
 * Generational randomized backfill proof that {@link FacetIdIndex} snapshots and restores correctly under a per-entity
 * savepoint (Ref: #1252). Because the index is a {@link io.evitadb.core.transaction.memory.TransactionalLayerProducer}
 * whose transactional entity-id bitmap is `Snapshotable`, the proof drives the index directly and asserts its logical
 * record contents (read via {@link LongRunningFacetIdIndexTest#snapshot(FacetIdIndex)}).
 *
 ...
 * commit), and asserts the record contents against the oracle captured at savepoint open. The transaction then commits
 ...
 */
@DisplayName("FacetIdIndex savepoint rollback/commit backfill (generational fuzz)")
@Tag(INDEXING)
@Tag(FACET)
@Tag(TRANSACTION)
class LongRunningSavepointFacetIdIndexTest implements TimeBoundedTestSupport {
	...
	void shouldRollBackFacetIdIndex(@Nonnull GenerationalTestInput input) {
		runFor(input, 1000, 0L, (random, iteration) -> {
			final FacetIdState state = new FacetIdState(random);
			assertSavepointRollbackRestores(
				state.index,
				tested -> state.applyRandomMutations(random, 1 + random.nextInt(MAX_OPS)),
				LongRunningFacetIdIndexTest::snapshot,
				tested -> {
	...
	void shouldCommitFacetIdIndex(@Nonnull GenerationalTestInput input) {
		runFor(input, 1000, 0L, (random, iteration) -> {
			final FacetIdState state = new FacetIdState(random);
	...
	/**
	 * A {@link FacetIdIndex} paired with an in-test model of its record contents (the set of entity ids) so randomized
	 ...
	 * transaction.
	 */
	private static final class FacetIdState {
		private static final int MAX_ENTITY_ID = 50;

		private final FacetIdIndex index = new FacetIdIndex(1);
		...
		FacetIdState(@Nonnull Random random) {
		...
		void applyRandomMutations(@Nonnull Random random, int count) {
			for (int i = 0; i < count; i++) {
				if (this.entityIds.isEmpty() || random.nextBoolean()) {
					addRandomFacet(random);
				} else {
		...
		/**
		 * Applies one guaranteed-visible change: adds a brand-new entity id drawn from a reserved sequence that random
		 * ops never touch, so the in-savepoint batch is never a no-op.
		 */
		void forceMutation() {
			final int entityId = ++this.forcedEntitySeq;
			this.index.addFacet(entityId);
			this.entityIds.add(entityId);
		}

		/**
		 * Adds a random not-yet-present entity id; bounded retries avoid an infinite spin when a random pick collides,
		 * and give up silently as a harmless no-op.
		 */
		private void addRandomFacet(@Nonnull Random random) {
			for (int attempt = 0; attempt < 10; attempt++) {
				final int entityId = random.nextInt(MAX_ENTITY_ID) + 1;
				if (this.entityIds.add(entityId)) {
					this.index.addFacet(entityId);
					return;
				}
			}
		...
		/**
		 * Removes a random present entity id, mirrored into the model; a no-op when the model is empty.
		 */
		private void removeRandomFacet(@Nonnull Random random) {
			if (this.entityIds.isEmpty()) {
				return;
			}
			final List<Integer> ids = new ArrayList<>(this.entityIds);
			final int entityId = ids.get(random.nextInt(ids.size()));
			this.index.removeFacet(entityId);
