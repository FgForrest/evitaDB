...
public final class JolHeapSize {
	...
	public static long ownedSize(@Nonnull Object instance, @Nonnull Object... sharedRoots) {
		...
		for (final Object sharedRoot : sharedRoots) {
			// set subtraction, not `totalSize() - totalSize()`: only objects genuinely reachable from BOTH are
			// dropped, so naming a root that this instance does not actually reach cannot silently shrink the result
