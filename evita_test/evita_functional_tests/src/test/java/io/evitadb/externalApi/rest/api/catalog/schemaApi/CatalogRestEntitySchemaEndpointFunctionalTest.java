...
class CatalogRestEntitySchemaEndpointFunctionalTest extends CatalogRestSchemaEndpointFunctionalTest {
			...
			.body(
				...
				equalTo(
					map()
						...
						.e(AttributeSchemaDescriptor.FILTER_CAPABILITIES_IN_SCOPES.name(), list())
	...
	void shouldChangeSortableAttributeCompoundSchema(Evita evita, RestTester tester) {
		...
		tester.test(TEST_CATALOG)
			...
			.executeAndExpectOkAndThen()
			.body(
				SortableAttributeCompoundsSchemaProviderDescriptor.SORTABLE_ATTRIBUTE_COMPOUNDS.name() + ".mySpecialCompound",
				equalTo(
					map()
