...
/**
 * Verifies the schema-refresh **coalescing contract** of {@link SystemGraphQLRefreshingObserver}:
 * per-mutation engine events ({@link ModifyCatalogSchemaMutation} / {@link SetCatalogMutabilityMutation})
 * must NOT trigger a per-mutation GraphQL schema rebuild, while a coalesced
 ...
 * is covered end-to-end by `SystemGraphQLSubscriptionsFunctionalTest`, which exercises a real
 * GraphQL server with real subscribers — those tests catch real schema-rebuild bugs that mock
 * verification cannot.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2026
 */
@DisplayName("SystemGraphQLRefreshingObserver schema-refresh coalescing")
@Tag(GRAPHQL)
@Tag(EXTERNAL_API)
@Tag(CDC)
@Tag(SCHEMA)
class SystemGraphQLRefreshingObserverTest {

	/** Catalog name used by every fake capture in this suite. */
	private static final String CATALOG_NAME = "catalog-name";

	/** Mocked GraphQL manager whose invocations are verified after each scenario. */
	private GraphQLManager graphQLManager;
	/** System under test — wired against the mocked manager. */
	private SystemGraphQLRefreshingObserver observer;

	@BeforeEach
	void setUp() {
		this.graphQLManager = mock(GraphQLManager.class);
		...
		when(this.graphQLManager.refreshCatalog(any())).thenReturn(true);

		this.observer = new SystemGraphQLRefreshingObserver(this.graphQLManager);
	...
	void shouldNotRefreshCatalogPerModifyCatalogSchemaMutation() {
		...
		verify(this.graphQLManager, never()).refreshCatalog(any());
	}
	...
	void shouldRefreshCatalogExactlyOncePerCatalogSchemaUpdated() {
		...
		verify(this.graphQLManager, times(1)).refreshCatalog(CATALOG_NAME);
	}
