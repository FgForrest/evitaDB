 ...
 * {@link TransactionalObjectBPlusTree} implementation. It exercises
 * insert, search, upsert, delete, iteration, rebalancing (steal and
 * merge), transactional semantics, tree structure visualization,
 * constructor validation, and randomized generational proof.
 *
...
class TransactionalObjectBPlusTreeTest
	...
	/**
	 * Verifies that the tree is in a consistent state and that both
	 * forward and reverse value iterators return the expected values.
	 *
	...
	/**
	 * Asserts that the forward value iterator of the tree returns
	 * values matching the given sorted key array in ascending order.
	 *
	...
	/**
	 * Asserts that the reverse value iterator of the tree returns
	 * values matching the given sorted key array in descending order.
	 *
	...
	/**
	 * Creates a random tree using the default block sizes and the
	 * given random seed and element count.
	 *
	...
	/**
	 * Creates a random tree with the given block size parameters,
	 * random seed, and element count.
	 *
	 ...
	 */
	@Nonnull
	private static TransactionalObjectBPlusTree<Integer, String>
	deleteAndVerify(
		@Nonnull TransactionalObjectBPlusTree<Integer, String> tree,
		@Nonnull AtomicReference<int[]> expectedArray,
		int keyToDelete
	) {
		final AtomicReference<TransactionalObjectBPlusTree<Integer, String>> result =
			new AtomicReference<>();
	...
	class InsertOperationsTest {
		...
		void shouldOverwriteDuplicateKeys() {
			final TransactionalObjectBPlusTree<Integer, String>
				bPlusTree = new TransactionalObjectBPlusTree<>(
	...
	class SearchOperationsTest {
		...
		void shouldReturnEmptyWhenSearchingInEmptyTree() {
			final TransactionalObjectBPlusTree<Integer, String>
				tree = new TransactionalObjectBPlusTree<>(
				3, Integer.class, String.class
			);
	...
	class StealOperationsTest {
		...
		void shouldStealFromLeftmostNode() {
			...
			final AtomicReference<TransactionalObjectBPlusTree<Integer, String>>
	...
	class MergeOperationsTest {
		...
		void shouldMergeWithLeftNode() {
			...
			final AtomicReference<int[]> expectedArray =
				new AtomicReference<>(testTree.plainArray());
			TransactionalObjectBPlusTree<Integer, String>
				tree = testTree.bPlusTree();

			tree = deleteAndVerify(
				tree, expectedArray, 98
			);
		...
		void shouldMergeWithRightNode() {
			...
			deleteAndVerify(
				testTree.bPlusTree(), expectedArray, 93
			);
		}
		...
		void shouldMergeCausingIntermediateParentToStealFromLeft() {
			...
			deleteAndVerify(
				testTree.bPlusTree(), expectedArray, 34
			);
		}
		...
		void shouldMergeCausingIntermediateParentToMergeLeft() {
			...
			tree = deleteAndVerify(
				tree, expectedArray, 32
			);
			tree = deleteAndVerify(
				tree, expectedArray, 34
			);
			tree = deleteAndVerify(
				tree, expectedArray, 35
			);
			tree = deleteAndVerify(
				tree, expectedArray, 37
			);
		...
		void shouldMergeCausingIntermediateParentToMergeRight() {
			...
			tree = deleteAndVerify(
				tree, expectedArray, 25
			);
			tree = deleteAndVerify(
				tree, expectedArray, 26
			);
			tree = deleteAndVerify(
				tree, expectedArray, 27
			);
	...
	class ForwardKeyIterationTest {
		...
		@Test
		@DisplayName(
			"iterates keys from exact existing key"
		)
		void shouldIterateThroughLeafNodeKeysLeftToRightFromExactPosition() {
			...
			final int startPos =
				insertionPosition.position();
			final Integer[] partialCopy =
				new Integer[
					plainFullArray.length - startPos
					];
			for (int i = startPos;
				 i < plainFullArray.length; i++) {
				partialCopy[i - startPos] =
			...
			final Integer[] reconstructedArray =
				new Integer[partialCopy.length];
			int index = 0;
			while (it.hasNext()) {
				reconstructedArray[index++] = it.next();
			}
		...
		@Test
		@DisplayName(
			"returns empty key iterator when "
				+ "exceeding bounds"
		)
		void shouldFailToIterateKeysLeftToRightThroughNonExistingValues() {
			...
			final Iterator<Integer> it =
				testTree.bPlusTree()
					.greaterOrEqualKeyIterator(1000);
			assertFalse(it.hasNext());

			final Iterator<Integer> it2 =
				testTree.bPlusTree()
					.greaterOrEqualKeyIterator(-1000);
			assertTrue(it2.hasNext());
			assertEquals(0, it2.next());

			final Iterator<Integer> it3 =
				testTree.bPlusTree()
					.greaterOrEqualKeyIterator(177);
	...
	class ForwardEntryIterationTest {
		...
		@Test
		@DisplayName(
			"iterates entries from exact existing key"
		)
		void shouldIterateThroughLeafNodeEntriesLeftToRightFromExactPosition() {
			final TreeTuple testTree =
				prepareRandomTree(42, 100);
			final Iterator<Entry<Integer, String>> it =
				testTree.bPlusTree()
					.greaterOrEqualEntryIterator(40);
			final int[] plainFullArray =
				testTree.plainArray();
			final InsertionPosition insertionPosition =
				ArrayUtils
			...
			final Entry<Integer, String>[] partialCopy =
				new Entry[
					plainFullArray.length - startPos
					];
			for (int i = startPos;
				 i < plainFullArray.length; i++) {
				partialCopy[i - startPos] = new Entry<>(
					plainFullArray[i],
					"Value" + plainFullArray[i]
				);
			}
			...
			//noinspection unchecked
			final Entry<Integer, String>[] reconstructedArray =
				new Entry[partialCopy.length];
			int index = 0;
			while (it.hasNext()) {
				reconstructedArray[index++] = it.next();
			}
		...
		@Test
		@DisplayName(
			"returns empty entry iterator when start "
				+ "key exceeds maximum"
		)
		void shouldFailToIterateEntriesLeftToRightThroughNonExistingValues() {
			...
			final Iterator<Entry<Integer, String>> it =
				testTree.bPlusTree()
					.greaterOrEqualEntryIterator(1000);
			assertFalse(it.hasNext());
		}
	...
	class ReverseValueIterationTest {
		...
		@Test
		@DisplayName(
			"iterates values backwards from exact "
				+ "existing key"
		)
		...
		@Test
		@DisplayName(
			"iterates values backwards from "
				+ "non-existing key position"
		)
		void shouldIterateThroughLeafNodeValuesRightToLeftFromExactNonExistingPosition() {
			...
			final int thePosition =
				insertionPosition.alreadyPresent()
					...
					: insertionPosition.position();
			final String[] partialCopy =
				new String[thePosition];
			for (int i = partialCopy.length - 1;
				 i >= 0; i--) {
				partialCopy[thePosition - i - 1] =
					"Value" + plainFullArray[i];
			}
		...
		@Test
		@DisplayName(
			"returns empty reverse iterator when "
				+ "below minimum"
		)
		void shouldFailToIterateValuesRightToLeftThroughNonExistingValues() {
			...
			final Iterator<String> it =
				testTree.bPlusTree()
					.lesserOrEqualValueIterator(-1000);
			assertFalse(it.hasNext());
			...
			final Iterator<String> it3 =
				testTree.bPlusTree()
					.lesserOrEqualValueIterator(177);
			assertTrue(it3.hasNext());
		...
		@Test
		@DisplayName(
			"iterates all elements backwards "
				+ "from maximum key"
		)
		void shouldIterateFromLastKeyReverse() {
			...
			final Iterator<String> it =
				testTree.bPlusTree()
					.lesserOrEqualValueIterator(lastKey);
			int count = 0;
			while (it.hasNext()) {
				it.next();
				count++;
		...
		@Test
		@DisplayName(
			"iterates one element backwards "
				+ "from minimum key"
		)
		void shouldIterateFromFirstKeyReverse() {
			...
			final Iterator<String> it =
				testTree.bPlusTree()
					.lesserOrEqualValueIterator(firstKey);
			assertTrue(it.hasNext());
	...
	class ReverseKeyIterationTest {
		...
		@Test
		@DisplayName(
			"iterates keys backwards from exact "
				+ "existing key"
		)
		...
		@Test
		@DisplayName(
			"returns empty reverse key iterator when "
				+ "below minimum"
		)
	...
	class ReverseEntryIterationTest {
		...
		@Test
		@DisplayName(
			"iterates entries backwards from exact "
				+ "existing key"
		)
		void shouldIterateThroughLeafNodeEntriesRightToLeftFromExactPosition() {
			...
			final Entry<Integer, String>[] partialCopy =
				new Entry[endPos + 1];
			for (int i = endPos; i >= 0; i--) {
				partialCopy[endPos - i] = new Entry<>(
					plainFullArray[i],
					"Value" + plainFullArray[i]
				);
			}
		...
		@Test
		@DisplayName(
			"returns empty reverse entry iterator when "
				+ "below minimum"
		)
		void shouldFailToIterateEntriesRightToLeftThroughNonExistingValues() {
			...
			final Iterator<Entry<Integer, String>> it =
				testTree.bPlusTree()
					.lesserOrEqualEntryIterator(-1000);
			assertFalse(it.hasNext());
		}
	...
	class TransactionalSemanticsTest {
		...
		void shouldHandleTransactionalLayerProducers() {
			//noinspection unchecked
			final TransactionalObjectBPlusTree<Integer, TransactionalList<String>>
				theTree =
				new TransactionalObjectBPlusTree<>(
					Integer.class,
		...
		@Test
		@DisplayName(
			"sees uncommitted modifications "
				+ "within transaction"
		)
	...
	class GenerationalProofTest {
		...
		@ParameterizedTest(
			name = "TransactionalObjectBPlusTreeTest "
				+ "should survive generational randomized "
				+ "test applying modifications on it"
		)
	...
	/**
	 * Holds a reference to a B+ tree together with the plain sorted
	 * integer key array that mirrors its contents.
	 *
	...
	/**
	 * Mutable state carried across iterations in the generational
	 * proof test.
	 *
