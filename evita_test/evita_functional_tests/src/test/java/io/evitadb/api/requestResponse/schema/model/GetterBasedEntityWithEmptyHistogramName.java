...
/**
 * Fixture entity declaring a single `@Histogram` entry with the default
 * (empty-string) `nameOfTheIndex` inside a non-empty `bucketed` array.
 *
 * The analyzer must reject such entries — every histogram inside a non-empty
 * `bucketed` array must declare a non-empty `nameOfTheIndex`.
 */
@Entity
public interface GetterBasedEntityWithEmptyHistogramName {
	...
	@Reference(
		...
		bucketed = {
			@Histogram(
				value = @Expression("$reference.referencedEntity?.attributes['x'] ?? 0.0")
			)
		}
	)
