...
class HierarchyContentTest {
	...
	void shouldDefaultParentsBehaviourToMatching() {
		...
		assertEquals(HierarchyParentsBehaviour.MATCHING, hierarchyContent(stopAt(distance(1))).getParentsBehaviour());
	...
	void shouldCreateWithParentsBehaviour() {
		...
		final HierarchyContent hierarchyContent2 = hierarchyContent(HierarchyParentsBehaviour.COMPLETE, stopAt(distance(1)));
		assertEquals(HierarchyParentsBehaviour.COMPLETE, hierarchyContent2.getParentsBehaviour());
		assertEquals(stopAt(distance(1)), hierarchyContent2.getStopAt().orElse(null));
		assertTrue(hierarchyContent2.getEntityFetch().isEmpty());

		final HierarchyContent hierarchyContent3 = hierarchyContent(HierarchyParentsBehaviour.COMPLETE, entityFetchAll());
	...
	void shouldConformToEqualsAndHashContractForParentsBehaviour() {
		...
		assertEquals(hierarchyContent(HierarchyParentsBehaviour.MATCHING).hashCode(), hierarchyContent().hashCode());
		assertEquals(
		...
		assertNotEquals(hierarchyContent(HierarchyParentsBehaviour.COMPLETE).hashCode(), hierarchyContent().hashCode());
	}

	@Test
	@DisplayName("cloneWithArguments() carries an explicit behaviour and falls back to the default on an empty array")
	void shouldCloneWithParentsBehaviourArgument() {
		final HierarchyContent original = hierarchyContent(
			HierarchyParentsBehaviour.COMPLETE, stopAt(distance(1)), entityFetchAll()
		);

		final HierarchyContent clonedWithoutArguments = (HierarchyContent) original.cloneWithArguments(new Serializable[0]);
	...
	void shouldNotFullyContainWhenParentsBehavioursDiffer() {
		final HierarchyContent complete = hierarchyContent(HierarchyParentsBehaviour.COMPLETE, entityFetch(attributeContent()));
