...
/**
 * Tests for {@link ReferenceSummary} verifying construction, applicability, getters,
 * copy/clone operations, visitor acceptance, and equality contract.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2021
 */
@DisplayName("ReferenceSummary constraint")
@Tag(CONTRACT)
@Tag(REQUIRE)
@Tag(REFERENCE)
class ReferenceSummaryTest {
	...
	class ConstructionTest {
		...
		void shouldCreateViaFactoryWithNoArguments() {
			final ReferenceSummary referenceSummary = referenceSummary();

			assertNotNull(referenceSummary);
			assertEquals(FacetStatisticsDepth.COUNTS, referenceSummary.getStatisticsDepth());
		}
		...
		void shouldCreateViaFactoryWithDepthAndRequirements() {
			...
			assertEquals(
				new ReferenceSummary(FacetStatisticsDepth.COUNTS, entityFetch(), entityGroupFetch(attributeContent("code"))),
				referenceSummary
			);
		}
		...
		void shouldCreateFromRequirements() {
			assertEquals(
				...
				new ReferenceSummary(FacetStatisticsDepth.COUNTS, new EntityFetchRequire[0])
			);
			assertEquals(
				referenceSummary(FacetStatisticsDepth.COUNTS, entityFetch(attributeContent("code"))),
				new ReferenceSummary(FacetStatisticsDepth.COUNTS, entityFetch(attributeContent("code")))
			);
			assertEquals(
				referenceSummary(FacetStatisticsDepth.COUNTS, entityFetch(attributeContent("code")), entityGroupFetch()),
				new ReferenceSummary(
		...
		void shouldDefaultToCountsWhenNullStatisticsDepth() {
			final ReferenceSummary referenceSummary = new ReferenceSummary(null);
	...
	class GetterTest {
		...
		@Test
		@DisplayName("should return reference entity requirement when present")
		void shouldReturnReferenceEntityRequirement() {
			assertEquals(
				entityFetch(attributeContent("code")),
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContent("code")))
					.getReferenceEntityRequirement().orElse(null)
			);
			assertEquals(
				entityFetch(attributeContent("code")),
				referenceSummary(
			...
			assertNull(
				referenceSummary(FacetStatisticsDepth.IMPACT, entityGroupFetch(attributeContent("name")))
					.getReferenceEntityRequirement().orElse(null)
			);
			assertNull(
				referenceSummary(FacetStatisticsDepth.IMPACT).getReferenceEntityRequirement().orElse(null)
			);
		}
		...
		void shouldReturnGroupEntityRequirement() {
			...
			assertNull(
				referenceSummary(FacetStatisticsDepth.IMPACT).getGroupEntityRequirement().orElse(null)
			);
		}
		...
		void shouldReturnFilterBy() {
			assertEquals(
				filterBy(entityPrimaryKeyInSet(1)),
				referenceSummary(FacetStatisticsDepth.IMPACT, filterBy(entityPrimaryKeyInSet(1)))
					.getFilterBy().orElse(null)
			);
			assertNull(
				referenceSummary(FacetStatisticsDepth.IMPACT).getFilterBy().orElse(null)
			);
		}
		...
		void shouldReturnFilterGroupBy() {
			assertEquals(
				...
				referenceSummary(FacetStatisticsDepth.IMPACT, filterGroupBy(entityPrimaryKeyInSet(1)))
					.getFilterGroupBy().orElse(null)
			);
			assertNull(
				referenceSummary(FacetStatisticsDepth.IMPACT).getFilterGroupBy().orElse(null)
			);
		}
		...
		void shouldReturnOrderBy() {
			...
			assertNull(
				referenceSummary(FacetStatisticsDepth.IMPACT).getOrderBy().orElse(null)
			);
		}
		...
		void shouldReturnOrderGroupBy() {
			...
			assertNull(
				referenceSummary(FacetStatisticsDepth.IMPACT).getOrderGroupBy().orElse(null)
			);
		}
	}
	...
	class ApplicabilityTest {
		...
		void shouldAlwaysBeApplicable() {
			assertTrue(referenceSummary().isApplicable());
			assertTrue(referenceSummary(FacetStatisticsDepth.IMPACT).isApplicable());
			assertTrue(referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContentAll())).isApplicable());
			assertTrue(
				referenceSummary(
					FacetStatisticsDepth.COUNTS,
					filterBy(entityPrimaryKeyInSet(1)),
	...
	class DefaultArgumentsTest {
		...
		void shouldExcludeCountsAsDefault() {
			...
			final Serializable[] argsExcludingDefaults = referenceSummary.getArgumentsExcludingDefaults();
		...
		void shouldKeepImpactAsNonDefault() {
			final ReferenceSummary referenceSummary = referenceSummary(FacetStatisticsDepth.IMPACT);
		...
		void shouldRecognizeCountsAsImplicit() {
			...
			assertTrue(referenceSummary.isArgumentImplicit(FacetStatisticsDepth.COUNTS));
			assertFalse(referenceSummary.isArgumentImplicit(FacetStatisticsDepth.IMPACT));
		}
	}
	...
	class TypeAndVisitorTest {
		...
		void shouldReturnRequireConstraintClassAsType() {
			assertEquals(RequireConstraint.class, referenceSummary().getType());
		}
		...
		void shouldAcceptVisitor() {
			...
			referenceSummary.accept(new ConstraintVisitor() {
				@Override
			...
			assertSame(referenceSummary, visited.get());
		}
	}
	...
	class CopyAndCloneTest {
		...
		void shouldCreateCopyWithNewChildren() {
			final ReferenceSummary original = referenceSummary(
				FacetStatisticsDepth.IMPACT,
				filterBy(entityPrimaryKeyInSet(1)),
				filterGroupBy(entityPrimaryKeyInSet(2)),
				orderBy(attributeNatural("code", OrderDirection.ASC)),
			...
			assertInstanceOf(ReferenceSummary.class, copy);
			final ReferenceSummary copyRef = (ReferenceSummary) copy;
		...
		void shouldCloneWithNewArguments() {
			final ReferenceSummary original = referenceSummary(FacetStatisticsDepth.COUNTS, entityFetch());
			final RequireConstraint cloned = original.cloneWithArguments(
				new Serializable[]{FacetStatisticsDepth.IMPACT}
			);

			assertNotSame(original, cloned);
			assertInstanceOf(ReferenceSummary.class, cloned);
			assertEquals(FacetStatisticsDepth.IMPACT, ((ReferenceSummary) cloned).getStatisticsDepth());
			assertTrue(((ReferenceSummary) cloned).getReferenceEntityRequirement().isPresent());
		}
	}
	...
	class ToStringTest {
		...
		void shouldProduceToStringWithDefaultDepth() {
			assertEquals("referenceSummary()", referenceSummary().toString());
		}
		...
		void shouldProduceToStringWithImpact() {
			assertEquals(
				"referenceSummary(IMPACT,entityFetch())",
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch()).toString()
			);
			assertEquals(
				"referenceSummary(IMPACT,entityFetch(attributeContent('code')),entityGroupFetch())",
				referenceSummary(
					FacetStatisticsDepth.IMPACT,
					entityFetch(attributeContent("code")),
					entityGroupFetch()
		...
		void shouldProduceToStringWithFilterAndOrder() {
			assertEquals(
				"referenceSummary(filterBy(entityPrimaryKeyInSet(1)),filterGroupBy(entityPrimaryKeyInSet(2)),"
	...
	class EqualityTest {
		...
		void shouldConformToEqualsAndHashContract() {
			assertNotSame(referenceSummary(), referenceSummary());
			assertEquals(referenceSummary(), referenceSummary());
			assertNotEquals(referenceSummary(), referenceSummary(FacetStatisticsDepth.IMPACT));
			assertEquals(
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContentAll()), entityGroupFetch()),
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContentAll()), entityGroupFetch())
			);
			assertNotEquals(
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContentAll()), entityGroupFetch()),
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContentAll()))
			);
			assertNotEquals(
				referenceSummary(
					FacetStatisticsDepth.COUNTS,
		...
		void shouldConformToHashCodeContract() {
			assertEquals(referenceSummary().hashCode(), referenceSummary().hashCode());
			assertEquals(
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContentAll()), entityGroupFetch())
					.hashCode(),
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContent()), entityGroupFetch())
					.hashCode()
			);
			assertNotEquals(
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContent()), entityGroupFetch())
					.hashCode(),
				referenceSummary(FacetStatisticsDepth.IMPACT, entityFetch(attributeContent())).hashCode()
			);
		}
	}
}
