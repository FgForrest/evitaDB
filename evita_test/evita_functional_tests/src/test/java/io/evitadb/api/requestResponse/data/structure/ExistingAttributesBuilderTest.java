...
/**
 * This test verifies contract of {@link ExistingAttributesBuilder}
 * class. Ie. tries modification on already existing
 * {@link Attributes} container.
 *
 * @author Jan Novotny (novotny@fg.cz), FG Forrest a.s. (c) 2021
 */
@DisplayName("ExistingEntityAttributesBuilder (attributes)")
class ExistingAttributesBuilderTest extends AbstractBuilderTest {
		...
		this.initialAttributes =
			new InitialEntityAttributesBuilder(
				new InternalEntitySchemaBuilder(
					...
					.withAttribute(
						"range", IntegerNumberRange.class
					)
					.withAttribute(
	...
	class OverridingAndSettingTest {
		...
		void shouldOverrideExistingAttributeWithNewValue() {
			final ExistingEntityAttributesBuilder builder =
				new ExistingEntityAttributesBuilder(
					PRODUCT_SCHEMA,
					ExistingAttributesBuilderTest.this
						.initialAttributes
				)
					.setAttribute("int", 10);
		...
		@Test
		@DisplayName("should add localized array")
		...
		@Test
		@DisplayName(
			"should ignore mutations producing equivalent"
		)
		...
		@Test
		@DisplayName("should succeed to add new attribute")
		...
		@Test
		@DisplayName("should update schema along the way")
		...
		@Test
		@DisplayName(
			"should set localized attribute"
		)
	...
	class RemovingTest {
		...
		@Test
		@DisplayName("should remove existing attribute")
		...
		@Test
		@DisplayName(
			"should remove existing localized attribute"
		)
		...
		@Test
		@DisplayName(
			"should handle remove and re-add"
		)
	...
	class DeltaMutationsTest {
		...
		@Test
		@DisplayName("should allow add mutation")
		...
		@Test
		@DisplayName(
			"should fail delta on non-existing attribute"
		)
		...
		@Test
		@DisplayName(
			"should allow combining multiple mutations"
		)
		void shouldAllowCombiningMultipleMutationsAtOnce() {
			...
			assertEquals(
				Integer.valueOf(5),
				newAttributes.getAttribute("newInt")
			);
		}

		@Test
		@DisplayName(
			"should allow combining create and remove"
		)
	...
	class SchemaValidationTest {
		...
		void shouldRespectExistingSchemaEarlyOnWhenSettingAttributes() {
			...
			final ExistingEntityAttributesBuilder
				attrBuilder =
				new ExistingEntityAttributesBuilder(
					updatedSchema, attrs
				);
			...
			assertThrows(
				InvalidMutationException.class,
				() -> attrBuilder.setAttribute("int", 1)
			);
			// try to set attributes correctly
			final Attributes<EntityAttributeSchemaContract>
				newAttrs = attrBuilder
			...
			assertEquals(
				2, newAttrs.getAttributeKeys().size()
			);
			assertEquals(
				"Hello",
				newAttrs.getAttribute("string")
			);
			assertEquals(
				2,
				newAttrs.getAttributeValue(
					new AttributeKey("string")
				).orElseThrow().version()
			);
			assertEquals(
				Integer.valueOf(7),
				newAttrs.getAttribute(
					"int", Locale.ENGLISH
				)
			);
			assertEquals(
		...
		@Test
		@DisplayName(
			"should allow adding if not strict"
		)
		void shouldAllowToAddNewAttributesIfVerificationIsNotStrict() {
			...
			// try to add with unsupported locale
			assertThrows(
				InvalidMutationException.class,
				() -> attrBuilder.setAttribute(
					"newLocalized",
			...
			assertEquals(
				4, updatedAttrs.getAttributeKeys().size()
			);
			assertEquals(
				"A",
				updatedAttrs.getAttribute("new")
			);
			assertEquals(
				"B",
				updatedAttrs.getAttribute(
					"newLocalized", Locale.ENGLISH
				)
			);
			assertEquals(
				"Hi",
				updatedAttrs.getAttribute("string")
			);
			assertEquals(
				Integer.valueOf(5),
				updatedAttrs.getAttribute(
		...
		void shouldRespectExistingSchemaEarlyOnWhenSettingAttributesButAllowAddingLocales() {
			...
			assertEquals(
				3, updatedAttrs.getAttributeKeys().size()
			);
			assertEquals(
				"Hello",
				updatedAttrs.getAttribute("string")
	...
	class ChangeSetAndIdentityTest {
		...
		@Test
		@DisplayName(
			"should fail to add incompatible mutation"
		)
		...
		@Test
		@DisplayName(
			"should ignore invalid mutation"
		)
		...
		@Test
		@DisplayName(
			"should build empty change set when no mods"
		)
