...
public class EntityFetchingFunctionalTest extends AbstractHundredProductsFunctionalTest {
	...
	void shouldReturnPaginatedReferences(Evita evita, List<SealedEntity> originalProducts) {
		final SealedEntity productWithMaxReferences = originalProducts
			.stream()
			.filter(it -> !it.getReferences(Entities.BRAND).isEmpty() && !it.getReferences(Entities.PARAMETER).isEmpty())
	...
	void shouldFetchAllReferencesIncludingNonManaged(Evita evita, List<SealedEntity> originalProducts) {
		...
		evita.queryCatalog(
			...
			session -> {
				...
				assertFalse(product.getReferences(REFERENCE_GALLERY).isEmpty(), "Gallery references should be present!");
				assertFalse(product.getReferences(Entities.CATEGORY).isEmpty(), "Category references should be present!");
				assertEquals(
	...
	@UseDataSet(HUNDRED_PRODUCTS)
	@Test
	void shouldFetchNamedReferenceSetsWithNonManagedReferenceFiltering(Evita evita, List<SealedEntity> originalProducts) {
		final SealedEntity targetProduct = findEntityByPredicate(
			originalProducts,
			it -> {
				final Collection<ReferenceContract> galleryRefs = it.getReferences(REFERENCE_GALLERY);
	...
	@UseDataSet(HUNDRED_PRODUCTS)
	@Test
	void shouldFetchManagedAndNonManagedReferencesWithAttributeFilters(Evita evita, List<SealedEntity> originalProducts) {
		final SealedEntity targetProduct = findEntityByPredicate(
			originalProducts,
			it -> {
				final Collection<ReferenceContract> galleryRefs = it.getReferences(REFERENCE_GALLERY);
	...
	void shouldReturnPaginatedNonManagedReferences(Evita evita, List<SealedEntity> originalProducts) {
		...
		assertTrue(totalGalleryCount >= 2, "Target product must have at least 2 gallery references for pagination test!");
		...
		evita.queryCatalog(
			...
			session -> {
				...
				assertEquals(totalGalleryCount, allReferencedKeys.size(), "All gallery references must be reachable via pagination!");
				return null;
			}
		);
	}
