...
public class CatalogCopySupport {
	...
	public static void replicateSchema(
		...
		final Map<String, EntitySchemaContract> schemasByName = new HashMap<>(sourceEntitySchemas.size());
		for (final EntitySchemaContract source : sourceEntitySchemas) {
			schemasByName.put(source.getName(), source);
		}
		for (final EntitySchemaContract source : sourceEntitySchemas) {
			final EntitySchemaBuilder builder = session.defineEntitySchema(source.getName());
			replicateReflectedReferences(source, builder, schemasByName);
			builder.updateVia(session);
		}
	}
	...
	private static void replicateReflectedReferences(
		@Nonnull final EntitySchemaContract source,
		@Nonnull final EntitySchemaBuilder builder,
		@Nonnull final Map<String, EntitySchemaContract> schemasByName
	) {
		for (final ReferenceSchemaContract reference : source.getReferences().values()) {
			if (!(reference instanceof ReflectedReferenceSchemaContract reflected)) {
				continue;
			}
			if (targetHasPartialFaceting(reflected, schemasByName)) {
				// A reflected reference cannot be defined against a conditionally faceted target: it may not
				// inherit the expression (its paths are direction-specific and resolve to the wrong entity
				// type in the reflected direction), and every explicit alternative is rejected in turn -
				// `nonFaceted(...)` is refused on an inherited schema, and declaring faceted scopes drags
				// indexed components into scopes where the reference is not indexed. The projection is
				// therefore omitted from the replica. It is read-only and lives on the *reflecting*
				// collection, so the write path of the collection that owns the original reference - the one
				// this harness measures - is unaffected.
				log.warn(
					"Reflected reference `{}.{}` omitted from the replica: its target `{}` is conditionally " +
						"faceted (facetedPartially) and the pair cannot be re-declared.",
					source.getName(), reflected.getName(), reflected.getReflectedReferenceName()
				);
				continue;
			}
			builder.withReflectedReferenceToEntity(
				...
				editor -> {
					...
					// Faceting is declared explicitly here even when the source inherits it. A reflected
					// reference may not inherit a `facetedPartially` expression whose paths are
					// direction-specific (`$reference.referencedEntity` resolves to the wrong entity type in
					// the reflected direction) - the engine rejects the definition outright. Declaring the
					// effective scopes explicitly breaks the inheritance and keeps the replica definable.
					// The replica's reflected reference is therefore *unconditionally* faceted where the
					// source was conditionally faceted; that shifts write cost on the reflecting collection
					// only, never on the collection owning the original reference.
					final Scope[] facetedScopes = scopesWhere(reflected::isFacetedInScope);
					if (!reflected.isFacetedInherited() && facetedScopes.length > 0) {
	...
	/**
	 * Returns `true` when the reference a reflected reference points at declares a `facetedPartially`
	 * expression in any scope - the case in which faceting may not be inherited.
	 *
	 * @param reflected     the reflected reference schema
	 * @param schemasByName all source entity schemas, by entity type
	 * @return `true` when the target reference is conditionally faceted
	 */
	private static boolean targetHasPartialFaceting(
		@Nonnull final ReflectedReferenceSchemaContract reflected,
		@Nonnull final Map<String, EntitySchemaContract> schemasByName
	) {
		final ReferenceSchemaContract target = resolveReflectedTarget(reflected, schemasByName);
		if (target == null) {
			return false;
		}
		for (final Scope scope : Scope.values()) {
			if (target.getFacetedPartiallyInScope(scope) != null) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Resolves the reference a reflected reference mirrors, or `null` when the owning collection is not part
	 * of the replicated set.
	 *
	 * @param reflected     the reflected reference schema
	 * @param schemasByName all source entity schemas, by entity type
	 * @return the mirrored reference schema, or `null` when it cannot be resolved
	 */
	@Nullable
	private static ReferenceSchemaContract resolveReflectedTarget(
		@Nonnull final ReflectedReferenceSchemaContract reflected,
		@Nonnull final Map<String, EntitySchemaContract> schemasByName
	) {
		final EntitySchemaContract targetSchema = schemasByName.get(reflected.getReferencedEntityType());
		return targetSchema == null
			? null
			: targetSchema.getReference(reflected.getReflectedReferenceName()).orElse(null);
	}
	...
	private static void applyReference(
		...
		// Indexed components decide which reduced index families exist for the reference
		// (REFERENCED_ENTITY / REFERENCED_GROUP_ENTITY). Applied after the index type, which preserves them.
		// Without this the replica silently gets the defaults and a whole index family can go missing.
		for (final Scope scope : Scope.values()) {
			// components are only definable where the reference is indexed at all - the engine rejects
			// "Indexed components must not be defined for scope `X` when the reference index type is NONE"
			if (reference.getReferenceIndexType(scope) == ReferenceIndexType.NONE) {
				continue;
			}
			final Set<ReferenceIndexedComponents> components = reference.getIndexedComponents(scope);
			if (!components.isEmpty()) {
				editor.indexedWithComponentsInScope(
					scope, components.toArray(new ReferenceIndexedComponents[0])
				);
			}
		}
		...
		// Conditional (partial) faceting. Omitting it turns a `facetedPartially` reference into an
		// unconditionally faceted one in the replica, which silently disables the whole deferred
		// expression re-evaluation path - so a benchmark taken on the replica cannot see any of its cost.
		for (final Scope scope : Scope.values()) {
			final Expression facetedPartially = reference.getFacetedPartiallyInScope(scope);
			if (facetedPartially != null) {
				editor.facetedPartiallyInScope(scope, facetedPartially);
			}
		}
