...
| [`RangeIndexFootprintProbe`](RangeIndexFootprintProbe.md) | how many range points does a real range index hold, and what does it therefore cost? | the occupancy that made the long-tree port pay is per catalog, and this is the before/after instrument |
| [`PriceIndexFootprintProbe`](PriceIndexFootprintProbe.md) | what do a catalog's price indexes cost, and how much sharing is there between super and reduced ones? | the sharing ratio is a property of the catalog, not a guarantee |
| [`AttributeIndexScaffoldingProbe`](AttributeIndexScaffoldingProbe.md) | what does an entity index cost before it holds anything? | the empty-map floor scales with index count, which differs per catalog by two orders of magnitude |
| [`CatalogIndexFootprintCensus`](CatalogIndexFootprintCensus.md) | which index structure carries the most heap, and how much of the resident heap does the accounting explain? | the ranking is per catalog shape, and the statistics API reports no per-structure breakdown to reuse |
