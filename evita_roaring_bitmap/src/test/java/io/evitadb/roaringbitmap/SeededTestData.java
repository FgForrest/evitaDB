...
public class SeededTestData {
	...
	public static PersistentRoaringBitmap randomBitmap(int maxKeys, double rleLimit, double denseLimit) {
		try {
	...
	public static PersistentRoaringBitmap randomBitmap(int maxKeys) {
		try {
	...
	private static PersistentRoaringBitmap randomBitmap(
	...
	public static class TestDataSet {
		...
		public TestDataSet withRunAt(int key) {
			assert key < 1 << 16;
			rleRegion(this.random).map(i -> (key << 16) | i).forEach(this.writer::add);
			return this;
		}

		public TestDataSet withArrayAt(int key) {
			assert key < 1 << 16;
			sparseRegion(this.random).map(i -> (key << 16) | i).forEach(this.writer::add);
			return this;
		}

		public TestDataSet withBitmapAt(int key) {
			assert key < 1 << 16;
			denseRegion(this.random).map(i -> (key << 16) | i).forEach(this.writer::add);
			return this;
		}

		public TestDataSet withRange(long minimum, long supremum) {
			this.writer.add(minimum, supremum);
			return this;
		}

		public PersistentRoaringBitmap build() {
