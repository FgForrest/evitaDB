...
/**
 * Regression tests for {@link PersistentRoaringBitmap#addOffset}, ported from the upstream
 * RoaringBitmap test suite. They pin down that shifting a bitmap applies the offset element-wise,
 * preserves cardinality and survives a serialize/deserialize round trip across a range of
 * container layouts and offsets, including known failure cases.
 */
@Execution(ExecutionMode.CONCURRENT)
@DisplayName("PersistentRoaringBitmap.addOffset")
public class TestConcatenation {
	...
	@MethodSource("params")
	public void testElementwiseOffsetAppliedCorrectly(PersistentRoaringBitmap bitmap, int offset) {
		int[] array1 = bitmap.toArray();
		for (int i = 0; i < array1.length; ++i) {
			array1[i] += offset;
		}
		PersistentRoaringBitmap shifted = PersistentRoaringBitmap.addOffset(bitmap, offset);
		assertArrayEquals(array1, shifted.toArray(), failureMessage(bitmap));
	}

	@ParameterizedTest(name = "{1}")
	@MethodSource("params")
	public void testCardinalityPreserved(PersistentRoaringBitmap bitmap, int offset) {
		PersistentRoaringBitmap shifted = PersistentRoaringBitmap.addOffset(bitmap, offset);
	...
	@ParameterizedTest(name = "{1}")
	@MethodSource("params")
	public void canSerializeAndDeserialize(PersistentRoaringBitmap bitmap, int offset) throws IOException {
		PersistentRoaringBitmap shifted = PersistentRoaringBitmap.addOffset(bitmap, offset);
		ByteArrayDataOutput out = ByteStreams.newDataOutput();
		shifted.serialize(out);
		PersistentRoaringBitmap deserialized = new PersistentRoaringBitmap();
		deserialized.deserialize(ByteStreams.newDataInput(out.toByteArray()));
		assertEquals(shifted, deserialized, failureMessage(bitmap));
	}

	private static PersistentRoaringBitmap read(String classPathResource) {
		try {
	...
	private static String failureMessage(PersistentRoaringBitmap bitmap) {
