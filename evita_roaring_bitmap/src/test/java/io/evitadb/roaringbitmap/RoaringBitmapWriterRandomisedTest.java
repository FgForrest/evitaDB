...
/**
 * Randomised tests, ported from the upstream RoaringBitmap test suite, verifying that
 * {@link RoaringBitmapWriter} builds a bitmap identical to {@link PersistentRoaringBitmap#bitmapOf}
 * across ordered inputs ranging from empty to ten million elements.
 */
@Execution(ExecutionMode.CONCURRENT)
@DisplayName("RoaringBitmapWriter (randomised, ordered input)")
public class RoaringBitmapWriterRandomisedTest {
	...
	public void getShouldFlushToTheUnderlyingBitmap_ConstantMemory(int[] values) {
		RoaringBitmapWriter<PersistentRoaringBitmap> writer = writer().constantMemory().get();
		writer.addMany(values);
		verify(writer.get(), values);
	}
	...
	private void verify(PersistentRoaringBitmap rb, int[] values) {
		PersistentRoaringBitmap baseline = PersistentRoaringBitmap.bitmapOf(values);
