 ...
 *
 * An ancestor may be present in the hierarchy index and still fail to yield a body: it may hold no data in the locale
 ...
 *
 * - {@link #MATCHING} — the chain is cut below the first ancestor whose requested body cannot be materialized. Every
 ...
 * - {@link #COMPLETE} — every ancestor is returned. One whose requested body cannot be materialized is returned as
 ...
 *
 * ## Why {@link #MATCHING} is the default
 *
 * {@link #MATCHING} is the backward-compatible value: it is the closer of the two to the behaviour that preceded this
 * argument, and the only one that never introduces a new exception for an existing caller. Choosing {@link #COMPLETE}
 ...
 *
 * A caller that wants the ancestors above an unmaterializable one has to ask for {@link #COMPLETE} explicitly, and
 * has to be prepared for a returned ancestor that carries no body.
 *
