...
/**
 * Implementation of the `ExpressionNode` interface that represents a minimum operation between two operands.
 *
 * This class takes two `ExpressionNode` operands and computes their minimum value when evaluated.
 * Both operands must evaluate to instances of `Number`.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2024
 */
@EqualsAndHashCode
public class MinOperator implements ExpressionNode {
	@Serial private static final long serialVersionUID = 8422657155033759778L;
	private final ExpressionNode leftOperator;
	private final ExpressionNode rightOperator;

	public MinOperator(@Nonnull ExpressionNode leftOperator, @Nonnull ExpressionNode rightOperator) {
	...
	public BigDecimal compute(@Nonnull PredicateEvaluationContext context) {
		...
		Assert.isTrue(
			...
			() -> new ParserException("Min function left operand must be number!")
		);
		final Serializable value2 = this.rightOperator.compute(context);
		Assert.isTrue(
			value2 instanceof Number,
			() -> new ParserException("Min function right operand must be number!")
		);
		...
		return convertedValue1.compareTo(convertedValue2) < 0 ? convertedValue1 : convertedValue2;
	}
	...
	public String toString() {
		return "min(" + this.leftOperator + ", " + this.rightOperator + ")";
	}
